...
class WeightTracker extends Component {
  ...
  componentDidMount = () => {
    ...
    if (user) {
      axios
        .get(
          `https://weighttracker-ffaf8.firebaseio.com/${user.displayName}/initialise.json`
        )
        .then(result => {
          this.setState(
            { initialiseData: result.data[Object.keys(result.data)[0]] },
            () => {
              axios
                .get(
                  `https://weighttracker-ffaf8.firebaseio.com/${user.displayName}/weights.json`
                )
                .then(result => {
                  ...
                  this.setState(
                    ...
                    () => {
                      ...
                      let originalStart = parseFloat(
                        this.state.initialiseData &&
                          this.state.initialiseData.initialWeight
                      );
                      const avg =
                        (this.state.initialiseData.goalWeight -
                          this.state.initialiseData.initialWeight) /
                        90;
                      for (
                        var index = 0;
                        ...
                        if (index === 0) {
                          ...
                          this.state.data[index]["DayDiff"] = parseInt(
                            (b - a) / (1000 * 60 * 60 * 24)
                          );
                          this.state.data[index]["weight2"] =
                            Number(this.state.data[index - 1].weight2) +
                            Number(avg * this.state.data[index].DayDiff);
                        }
                      ...
                      var valueAxis1 = chart.yAxes.push(
                      ...
                      var bullet3 = series3.bullets.push(
                      ...
                      var bullet4 = series4.bullets.push(
                        new am4charts.CircleBullet()
                      );
    ...
    axios
      ...
      .then(res => {
        ...
        Object.keys(result).map(function(key) {
          if (
            result[key].date === data.date &&
            result[key].weight === data.weight
          ) {
            axios
              .delete(
                `https://weighttracker-ffaf8.firebaseio.com/${user.displayName}/weights/${key}.json`
              )
              .then(() => window.location.reload());
          }
        });
  ...
  render() {
    ...
    const sortedDataSet = sortedData.slice(-1)[0];
    ...
    return (
      <div className="App">
        <div className="header-section">
          <header>Welcome {user && user.displayName}</header>
          <div className="side">
            <div className="section first">
              {initialiseData &&
                `Initial weight: ${initialiseData.initialWeight} KGS`}
            </div>
            <div className="section">{`${duration} days been`}</div>
          </div>
        </div>
        <div className="flex">
          <div className={`inner-flex data-0`}>
            <div className="flexer">
              <div className="title">Current Weight</div>
              <div className="subtitle">
                {`${sortedDataSet && sortedDataSet.weight} KGS`}
              </div>
              <div className="small">
                {sortedDataSet &&
                  "Last updated: " +
                    moment(sortedDataSet.date).format("DD/MM/YYYY")}
              </div>
            </div>
            <div className="flexer">
              <div className="title">Goal Weight</div>
              <div className="subtitle">
                {`${initialiseData && initialiseData.goalWeight} KGS`}
              </div>
              <div className="small">
                {initialiseData &&
                  moment(initialiseData.goalDate).format("DD/MM/YYYY")}
              </div>
            </div>
          </div>
          <div className={`inner-flex data-2`}>
            <div className="flexer">
              <div className="title">Days remaining</div>
              <div className="subtitle">
                {this.calculateDate(initialiseData && initialiseData.goalDate)}
              </div>
            </div>
            <div className="flexer">
              <div className="title">Weight remaining</div>
              <div className="subtitle">
                {sortedDataSet &&
                  initialiseData &&
                  parseFloat(
                    Number(initialiseData.goalWeight) -
                      Number(sortedDataSet.weight)
                  ).toFixed(2) + " KGS"}
              </div>
              <div className="small">
                {sortedDataSet &&
                  ...
                  parseFloat(
                    parseFloat(
                      initialiseData.goalWeight - sortedDataSet.weight
                    ).toFixed(2) /
                      (this.calculateDate(
        ...
        <div className="flex2">
          <div className={`inner-flex data-0`}>
            <div className="title">Weight expected to be today</div>
            <div className="subtitle">
              {sortedDataSet &&
                `${parseFloat(sortedDataSet.weight2).toFixed(2)} KGS`}
            </div>
          </div>
          <div className={`inner-flex data-1`}>
            <div className="title">Overall weight change</div>
            <div className="subtitle">
              {`${parseFloat(
                Number(sortedDataSet && sortedDataSet.weight) -
                  Number(initialiseData && initialiseData.initialWeight)
              ).toFixed(2)} KGS`}
            </div>
          </div>
          <div className={`inner-flex data-2`}>
            <div className="title">Weeks remaining</div>
            <div className="subtitle">
              {parseInt(
                this.calculateDate(initialiseData && initialiseData.goalDate) /
                  7
        ...
        <div className="wrapped">
          <div className="flexed">
            <p>Enter weight</p>
            <TextField label="Enter Weight" className="field">
              <Input
                type="number"
              ...
              sortedData.map((data, arr) => (
                <div className="history">
                  ...
                  <div className="weight">
                    {arr !== 0 && (
                      <span onClick={() => this.deleteWeight(data)}>
                        Delete weight
                      </span>
                    )}
                    {`${data.weight} KGS`}
        ...
        <div className="wrapped">
          <div className="flexed">
            ...
            {sortedWeeks &&
              sortedWeeks.map((weeks, i, arr) => (
                <div
                  className={`inner-flex ${arr.length - 1 === i ? "last" : ""}`}
                >
                  <div className="title">{`Change in week ${i + 1}`}</div>
                  <div className="arrow">
                    <div>
                    ...
                    <div className="bold">
                      {parseFloat(
                        weeks.slice(-1)[0].weight - weeks[0].weight
                      ).toFixed(2) + " KGS"}
                    </div>
                  </div>
                </div>
